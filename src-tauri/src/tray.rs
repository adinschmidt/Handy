...
fn compute_desired(app: &AppHandle, icon_state: TrayIconState) -> TrayDesired {
    ...
    TrayDesired {
        ...
        menu: MenuInputs {
            ...
            warning,
            model_loaded,
            selected_provider: settings.selected_transcription_provider,
...
fn build_menu(app: &AppHandle, inputs: &MenuInputs) -> tauri::Result<(Menu<tauri::Wry>, String)> {
    ...
    let menu = if inputs.busy {
        ...
        for (id, name) in &inputs.downloaded_models {
            let is_active = inputs.selected_provider == settings::TranscriptionProvider::Local
        ...
        for (provider, id, label, enabled) in [
            ...
            let item = CheckMenuItem::with_id(
                ...
                label,
                enabled,
                inputs.selected_provider == provider,
                None::<&str>,
            )?;
            model_submenu.append(&item)?;
        }
...
mod tests {
    ...
    #[test]
    fn provider_changes_invalidate_the_tray_menu() {
        let local = inputs(false);
        let mut cloud = local.clone();
        cloud.selected_provider = crate::settings::TranscriptionProvider::ElevenlabsScribe;
